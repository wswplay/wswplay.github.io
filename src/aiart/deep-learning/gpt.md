...
这些是优化或结构变体，不是“注意力机制类型”的根本变化，模型的基础机制仍是**多头缩放点积自注意力**。

### FlashAttention

`FlashAttention` 是一种 **内存访问优化 + CUDA kernel 重写** 的实现技术，使得 **Transformer 注意力机制的前向和反向传播能在显存受限的条件下高速运行**。

- 原始注意力的 softmax 是全局矩阵乘法，计算量大，且内存读写频繁。
- FlashAttention 将 softmax 的计算 **融合进 GEMM（矩阵乘法）流程中**，**避免中间结果落地（写入显存）**，从而节省大量内存并加快速度。

### 稀疏注意力

`稀疏注意力`指的是在注意力矩阵中**只计算部分位置**的注意力权重，从而降低计算复杂度。常见稀疏策略包括：

| 策略类型       | 简介                                    |
| -------------- | --------------------------------------- |
| Block Sparse   | 只对局部窗口或远程块建立注意力连接      |
| Local + Global | 部分 token 有全局视野，其余只看局部邻居 |
| Routing-based  | 基于 token 内容动态决定注意位置         |
| Performer 等   | 用核函数逼近注意力矩阵，复杂度降到线性  |

稀疏注意力可将注意力复杂度从 O(n²) 降为 O(n)，更适合处理 **长上下文序列（如 GPT‑4‑Turbo 所宣称的 128k 上下文）**。
